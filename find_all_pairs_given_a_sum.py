...
def findPairs(a:list, b:list, target_sum:int):
#     create two hash maps, iterate over list, use difference to find if number is in the hashmap, sort, and return
    hash_map_b = dict()
    result_map = dict()
    for i in range (len(b)):
        hash_map_b[b[i]] = 1
    for i in range (len(a)):
        potential_pair = target_sum - a[i]
        if potential_pair in hash_map_b:
            result_map[a[i]] = potential_pair
...
if __name__ == "__main__":
    findPairs([1, 2, 4, 5, 7], [5, 6, 3, 4, 8], 9)
    findPairs([-1, -2, 4, -6, 5, 7], [6, 3, 4, 0], 8)
